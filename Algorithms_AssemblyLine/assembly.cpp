...
#define random() rand()%10+1; //Returns a random Number with the current time as seed
#define min(X,Y) ((X) < (Y) ? (1) : (0)) // Returns minimum of two numbers
...
//Displays Results Generated by the Algorithm
...
void Path(bool fastest1[],bool fastest2[],assembly LINE1,assembly LINE2,int n)
{
	...
	for(int j=1;j<n;j++)
	{
		...
		if(min((line1[j-1]+LINE1.STATIONARRAY[j]),(line2[j-1]+LINE2.TRAVERSALCOST[j-1]+LINE1.STATIONARRAY[j])))
		{
		...
		if(min((line2[j-1]+LINE2.STATIONARRAY[j]),(line1[j-1]+LINE1.TRAVERSALCOST[j-1]+LINE2.STATIONARRAY[j])))
		{
	...
	if (min((line1[n-1]+LINE1.EXITCOST),(line2[n-1]+LINE2.EXITCOST)))
	{
